...
test('固定 OAuth 回跳地址，不接受外部 returnTo', () => {
  assert.equal(adminReturnUrl(env), 'https://ice11123.github.io/GUET_428/admin/');
});
...
test('public status 无需管理员 Cookie 且返回脱敏仓库和部署状态', async () => {
  ...
  globalThis.fetch = async (url) => {
    ...
    if (target.includes('/actions/workflows/deploy.yml/runs')) return Response.json({ workflow_runs: [{ status: 'completed', conclusion: 'success', updated_at: '2026-08-13T12:00:00Z', html_url: 'https://github.com/ice11123/GUET_428/actions/runs/2' }] });
...
test('public status 上游失败时回退旧缓存并标记 stale', async () => {
  ...
  try {
    const old = { ok: true, checkedAt: '2026-08-13T10:00:00Z', stale: false, worker: { ok: true }, repository: { ok: true, owner: 'ice11123', name: 'GUET_428', branch: 'main', headSha: 'old-head', commitUrl: 'https://github.com/commit' }, deployment: { status: 'pending', updatedAt: null, url: null } };
...
test('api/status 返回仓库 HEAD 与最近部署且不泄露 token', async () => {
  ...
  globalThis.fetch = async (url) => {
    ...
    if (target.includes('/actions/workflows/deploy.yml/runs')) return Response.json({ workflow_runs: [{ status: 'completed', conclusion: 'success', updated_at: '2026-08-13T10:00:00Z', html_url: 'https://github.com/ice11123/GUET_428/actions/runs/1' }] });
    return new Response('not found', { status: 404 });
  };
  try {
