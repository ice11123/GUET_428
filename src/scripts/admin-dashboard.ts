...
const PENDING_CLOUD_PUBLISH_KEY = 'guet-428-pending-cloud-publish-v1';
const PENDING_CLOUD_DELETE_KEY = 'guet-428-pending-cloud-delete-v1';
const LEGACY_ADMIN_SESSION_KEY = 'guet-428-cloud-session-v1';
...
async function checkBasicStatus() {
  ...
  const api = endpoint();
  if (!api) {
    setCard('worker', 'waiting', '尚未接入发布服务', 'GUET_428 暂不部署 Worker');
    return false;
  }
...
async function checkFullStatus() {
  ...
  if (!workerOk) {
    ...
    setCard('repository', 'waiting', '实验室仓库未接入发布', '目标：ice11123/GUET_428 main');
  ...
  setCard('repository', 'checking', '正在读取实验室仓库', '目标：ice11123/GUET_428 main');
