...
test('左侧栏移除个人资料并保留目录、标签目录，文章 TOC 独立位于右栏', () => {
  ...
  assert.doesNotMatch(sidebar, /data-sidebar-tab="profile"|profile-avatar|SocialLinks/);
...
test('实验室身份替代个人元素，亮色主题采用浅蓝底并保留红金蓝强调色', () => {
  ...
  const lightTheme = readSource('styles/themes/light-blue.css');

  assert.match(constants, /428强弱电综合实验室/);
  assert.match(constants, /桂林电子科技大学/);
  assert.match(constants, /电气工程及其自动化/);
...
test('主页默认隐藏统一侧栏并移除高饱和巨大字占位', () => {
  ...
  assert.match(sidebar, /data-sidebar-tab="catalog"[\s\S]*data-sidebar-tab="tags"/);
  assert.doesNotMatch(sidebar, /data-sidebar-tab="profile"|aria-label="个人"/);
