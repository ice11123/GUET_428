---
title: "01｜认识实验室 TI_CAR：它能做什么，代码怎样跑"
...
完成通用入门课程后，看到工程里的 `APP`、`BSP`、PID、PWM、编码器、循迹和一大堆引脚，仍然可能不知道从哪里下手。这篇文章专门介绍实验室现有的 TI 小车，帮助你把基础概念对应到真实工程。

这套教程不会要求你先把整个工程背下来。我们的目标是按一条可验证的路线，逐步做到：**看懂系统、让单个电机安全转动、读到编码器、闭环控制速度，最后再进入循迹和整车任务。**

> [!IMPORTANT]
> 本系列核对的是用户提供资料根目录中的 `TI_CAR_test1_6.7` 工作副本。旧 README、打包快照和硬件说明中仍有历史描述；发生冲突时，本文会优先依据当前构建配置与实际调用代码，并把仍需上板确认的内容明确标出。

## 这是一篇作品介绍，不是工程文件清单

TI_CAR 是一辆基于 MSPM0G3519 的双轮差速实验小车。它把供电、电机驱动、编码器测速、循迹输入、速度闭环、定距控制、显示和调试通信组合在一套工程中，适合用来学习“传感—决策—执行—反馈”的完整链路。

阅读本系列时要区分三种证据：

| 状态 | 含义 | 典型证据 |
| --- | --- | --- |
| 已配置 | 外设或引脚出现在工程配置中 | `.cproject`、`empty.syscfg` |
| 已接入主流程 | 初始化、任务或控制链确实调用了它 | 当前 `.c/.h` 调用关系 |
| 已上板验证 | 在指定接线、供电和版本下观察到结果 | 测量记录、波形、日志或视频 |

前两项可以从仓库判断，第三项不能只靠阅读代码代替。新生接手工程时，应把每次上板结果补成可追溯记录。
...
当前主流程读取第 4～8 路五个循迹输入。源码把低电平解释为检测到黑线，再将各传感器位置加权为横向偏差，用于修正左右目标速度。更换模块或接线后，仍要先用遮挡实验重新确认电平极性。
...
- 当前工程的活动构建配置指向 MSPM0G3519；判断硬件型号时，应以构建配置、SysConfig 和实际调用代码为准，而不是沿用旧文件名或历史说明。
- BSP 层负责把引脚和外设封装成稳定接口，APP 层负责状态、调度和控制逻辑。目标速度、编码器反馈与 PWM 指令沿这条链路逐级传递。
- 推荐按“停机路径 → 最小输出 → 编码器 → 速度闭环 → 循迹 → 整车任务”的顺序学习，先让每一层可验证，再逐步组合。
- 源码存在不代表功能已经启用；宏开关、初始化调用和任务登记都需要核对。当前循迹虽保留 12 个位置，但实际主流程只读取第 4～8 路。
- 仓库能够证明配置与调用关系，不能自动证明当前实物已经通过测试；实机结论必须同时记录硬件、供电、固件版本与观察证据。
